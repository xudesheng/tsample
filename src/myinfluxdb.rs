...
impl MyInfluxClient{
    pub fn new(data_target: &TestDataDestination) -> MyInfluxClient{
        ...
        let url = match data_target.using_udp {
            true => {
                format!("{}:{}", data_target.server_name, data_target.port)
            },
            false => {
                ...
                format!("{}://{}:{}", protocol, data_target.server_name,data_target.port)
            }
        };
        ...
        MyInfluxClient{
            ...
            url: url.to_string(),
            user: match &data_target.user {None=> None, Some(user)=>Some(user.to_string()),},
            password: match &data_target.password {None=>None, Some(pass) =>Some(pass.to_string())},
