...
pub fn samping_one_time(testid: &str, o_sampling: &OneTimeTest) -> Result<Point, Box<Error>>{
    //meansurement name
...
pub fn sampling_repeat(testid: &str, r_sampling: &RepeatTest) -> Result<Point, Box<Error>>{
    ...
    if r_sampling.mem_info_five {point.add_field("mem_info_one", Value::Float(load.five));}
    if r_sampling.mem_info_fifteen {point.add_field("mem_info_one", Value::Float(load.fifteen));}
    ...
    if r_sampling.disk_free {point.add_field("disk_total", Value::Integer(disk.free as i64));}
...
fn construct_headers(app_key:&str) -> Result<HeaderMap, Box<Error>> {
...
pub fn sampling_thingworx(twx_server: &ThingworxServer) -> Result<Vec<Point>, Box<Error>>{
